...
class Env:
    ...
    def step(self, guess: Tensor) -> Tensor:
        dif: Tensor = (self._scale_factor * (self._number - guess)).square()
        reward = tensor(1000., requires_grad=True) - dif
...
class AltAgent:
    ...
    def __init__(self, env: Env, network: AltNetwork):
        self._env = env
        self._network = network
        self._optimizer = Adam(network.parameters())
